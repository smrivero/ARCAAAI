...
function sanitizeCuit(raw: string): string {
  return raw.replace(/\D/g, "");
}

function logCredPreview(label: string, value: string): void {
...
export class WsfeError extends Error {
  override readonly name = "WsfeError";
  constructor(
    message: string,
 ...
 */
export function buildSoapEnvelope(methodName: string, body: string): string {
...
function extractSoapFaultString(xml: string): string | null {
...
function findFirstKeyDeep(obj: unknown, key: string): unknown {
...
function wsfeAfipStructuredErrors(parsed: Record<string, unknown>): string[] {
  const errsNode = findFirstKeyDeep(parsed, "Errors");
  if (!errsNode || typeof errsNode !== "object") {
    return [];
  }
  const rec = errsNode as Record<string, unknown>;
  let err = rec["Err"];
  if (err === undefined) {
    return [];
  }
 ...
 */
export async function callWSFEV1(soapEnvelope: string, soapAction: string): Promise<string> {
  const url = getRequiredEnv("ARCA_WSFEV1_URL_DEV");

  console.log("[wsfe] POST", url);
  console.log("[wsfe] SOAPAction:", soapAction);

  const res = await axios.post<string>(url, soapEnvelope, {
    headers: {
  ...
  if (res.status !== 200) {
    ...
    throw new WsfeError(
      fault ?? `WSFE HTTP ${String(res.status)}`,
      body.slice(0, 800),
    );
  }
...
function parseSoapToRecord(xml: string): Record<string, unknown> {
  try {
    return responseParser.parse(xml) as Record<string, unknown>;
  } catch (e) {
...
export function parseWsfeSoapXml(xml: string): Record<string, unknown> {
...
function requireSoapRecord(value: unknown, ctx: string): Record<string, unknown> {
 ...
 */
function getFeCompUltimoAutorizadoResult(parsed: Record<string, unknown>): Record<string, unknown> {
  ...
  const response = requireSoapRecord(
    body["FECompUltimoAutorizadoResponse"],
    "FECompUltimoAutorizado: falta FECompUltimoAutorizadoResponse.",
  );
  return requireSoapRecord(
    response["FECompUltimoAutorizadoResult"],
    "FECompUltimoAutorizado: falta FECompUltimoAutorizadoResult.",
  );
}

function getFeCompConsultarSoapResult(parsed: Record<string, unknown>): Record<string, unknown> {
  ...
  const response = requireSoapRecord(
    body["FECompConsultarResponse"],
    "FECompConsultar: falta FECompConsultarResponse.",
  );
  return requireSoapRecord(response["FECompConsultarResult"], "FECompConsultar: falta FECompConsultarResult.");
}

function throwIfFeCompUltimoResultHasAfipErrors(result: Record<string, unknown>): void {
  const errorsNode = result["Errors"];
  if (errorsNode === undefined || errorsNode === null) {
    return;
  }
  const msgs = wsfeAfipStructuredErrors({ Errors: errorsNode } as Record<string, unknown>);
  if (msgs.length > 0) {
    throw new WsfeError(
      `FECompUltimoAutorizado: AFIP devolvió errores en FECompUltimoAutorizadoResult: ${msgs.join("; ")}`,
      result,
    );
  }
}
...
function normalizeConsultarResultGet(node: unknown): Record<string, unknown> | null {
...
function feCompConsultarResultErrorMessages(result: Record<string, unknown>): string[] {
  const errorsNode = result["Errors"];
  if (errorsNode === undefined || errorsNode === null) {
    return [];
  }
  return wsfeAfipStructuredErrors({ Errors: errorsNode } as Record<string, unknown>);
}

function consultarHasMeaningfulResultGet(rg: Record<string, unknown> | null): rg is Record<string, unknown> {
...
function isConsultarSkippedNotFound(errs: string[]): boolean {
  if (errs.length === 0) {
    return false;
  }
  const text = errs.join(" ").toLowerCase();
  if (
    /inexistente|no existe|no se encontr|sin comprobante|sin comprobantes|no corresponde un comprobante/.test(text)
  ) {
    return true;
  }
...
function interpretFeCompConsultarSoap(parsed: Record<string, unknown>): Record<string, unknown> | null {
...
function voucherCbteFchInRange(
 ...
 */
export async function listWsfeVouchers(params: ListWsfeVouchersParams): Promise<ListWsfeVouchersResponseOk> {
  const limit =
    params.limit === undefined
      ? DEFAULT_WSFE_LIST_LIMIT
      : Math.min(Math.max(1, params.limit), MAX_WSFE_LIST_LIMIT);
  const ultimoParsed = await feCompUltimoAutorizado(
    { ptoVta: params.ptoVta, cbteTipo: params.cbteTipo },
    { logCredentialPreview: false, quietConsole: true },
  );
  ...
  let scans = 0;
  for (
    let n = lastVoucherNumber;
    n >= 1 &&
      collected.length < limit &&
      scans < MAX_WSFE_LIST_SCAN_ITERATIONS;
    n -= 1, scans += 1
  ) {
  ...
  const vouchers =
    params.fromDate === undefined && params.toDate === undefined
      ? collected
...
export const createVoucherBodySchema = z
  ...
  .refine(
    (d) => d.condicionIVAReceptorId !== undefined || d.docTipo === 99,
    {
      message:
        "condicionIVAReceptorId es obligatorio salvo DocTipo 99 (consumidor final): en ese caso puede omitirse y se asume 5.",
...
function todayYyyyMmDdArgentina(): string {
...
export function extractCbteNroFromUltimoResponse(parsed: Record<string, unknown>): number {
  const result = getFeCompUltimoAutorizadoResult(parsed);
...
function normalizeErrList(errNode: unknown): Array<Record<string, unknown>> {
...
function collectAllErrMessages(obj: unknown): string[] {
  const out: string[] = [];
...
function firstFecaDetLikeResponse(raw: Record<string, unknown>): Record<string, unknown> | null {
  const preferOrder = ["FECAEDetResponse", "FECAEADetailResponse"] as const;
...
function observacionesNodeToStrings(observationsNode: unknown): string[] {
  ...
  const rec = observationsNode as Record<string, unknown>;
  const obsEl = rec["Obs"];
  const out: string[] = [];
...
export function distillFecaSolicitarResponse(raw: Record<string, unknown>): FecaSolicitarResultView {
  ...
  if (detail) {
    ...
    observations.push(
      ...observacionesNodeToStrings(detail["Observaciones"]),
      ...observacionesNodeToStrings(detail["Observations"]),
    );
  }
...
function buildAlicIvaXml(rows: CreateVoucherBody["iva"]): string {
  return rows
    .map(
      (row) => `          <ar:AlicIva>
...
function buildFeCAEReqInner(input: CreateVoucherBody, nextNumber: number, cbteFch: string): string {
