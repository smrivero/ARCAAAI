function onlyDigitsIss(s) {
    return s.replace(/\D/g, "");
}
function trimmedOrUndef(raw) {
    if (raw === undefined) {
        return undefined;
    }
    const s = raw.trim();
...
export function copyTypesFromQueryParam(copiesRaw) {
...
export function parseVoucherPdfQueryMetadata(q) {
    const pick = (camel, snake) => trimmedOrUndef(q[camel]) ?? trimmedOrUndef(q[snake]);
    const meta = {
...
export function applyVoucherPdfMetadataToPayload(base, meta) {
    ...
    if (meta.itemDescription !== undefined && items.length >= 1) {
        items = items.map((it, i) => i === 0 ? { ...it, description: meta.itemDescription } : it);
    }
    return {
        ...base,
        copyTypes,
...
function bodyStringOverride(raw) {
    if (raw === undefined) {
        return undefined;
    }
    const t = raw.trim();
 ...
 */
export function mergeVoucherPdfPostIntoPayload(base, body) {
...
//# sourceMappingURL=invoice-pdf-merge.js.map