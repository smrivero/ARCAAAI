import { extractCaeFromConsultarResult, mapFeCompConsultarToInvoicePdfPayload, } from "../arca/fecomp-consultar-to-invoice-pdf.js";
import { createVoucherBodySchema, feCAESolicitar, feCompConsultar, feCompUltimoAutorizado, listWsfeVouchers, WsfeError, } from "../arca/wsfev1.js";
...
import { applyVoucherPdfMetadataToPayload, mergeVoucherPdfPostIntoPayload, parseVoucherPdfQueryMetadata, } from "../pdf/invoice-pdf-merge.js";
import { getWsaaTicketAccess, WsaaAlreadyAuthenticatedError, } from "../arca/wsaa.js";
const TA_CONFLICT_MESSAGE = "AFIP indicates a TA is already valid for this credential, but this backend has no usable copy (no valid ./tmp/ta.json). Wait until AFIP TTL expires or place a TA JSON file there after a successful login elsewhere.";
function wsfeErrorHttpStatus(err) {
    const m = err.message;
    if (m.includes("tmp/ta.json") ||
...
export async function arcaRoutes(app) {
    ...
    app.get("/wsaa/login", async (request, reply) => {
        try {
            ...
            request.log.error(err instanceof Error
                ? { errName: err.name, errMessage: err.message }
                : { err: String(err) }, "GET /arca/wsaa/login failed");
    ...
    app.get("/wsfe/last-voucher", async (request, reply) => {
        try {
            ...
            request.log.error(err instanceof Error
                ? { errName: err.name, errMessage: err.message }
                : { err: String(err) }, "GET /arca/wsfe/last-voucher failed");
            const message = err instanceof Error ? err.message : String(err);
            return await reply.status(500).send({ ok: false, error: message });
        }
    });
    ...
    app.get("/wsfe/vouchers", async (request, reply) => {
        try {
            ...
            if (ptoVtaRaw === undefined ||
                ...
                return await reply.status(400).send({
                    ok: false,
                    error: "Query inválida: `ptoVta` y `cbteTipo` son obligatorios (enteros, ptoVta 1–99999, cbteTipo ≥ 1).",
                });
            }
            let limitParsed;
            ...
            request.log.error(err instanceof Error
                ? { errName: err.name, errMessage: err.message }
                : { err: String(err) }, "GET /arca/wsfe/vouchers failed");
            const message = err instanceof Error ? err.message : String(err);
            return await reply.status(500).send({ ok: false, error: message });
        }
    });
    ...
    app.get("/wsfe/voucher-pdf", async (request, reply) => {
        try {
            ...
            if (ptoVtaRaw === undefined ||
                ...
                return await reply.status(400).send({
                    ok: false,
                    error: "Query inválida: `ptoVta`, `cbteTipo` y `cbteNro` son obligatorios (enteros positivos).",
                });
            }
            ...
            request.log.error(err instanceof Error
                ? { errName: err.name, errMessage: err.message }
                : { err: String(err) }, "GET /arca/wsfe/voucher-pdf failed");
            const message = err instanceof Error ? err.message : String(err);
            return await reply.status(500).send({ ok: false, error: message });
        }
    });
    ...
    app.post("/wsfe/voucher-pdf", async (request, reply) => {
        ...
        try {
            ...
            request.log.error(err instanceof Error
                ? { errName: err.name, errMessage: err.message }
                : { err: String(err) }, "POST /arca/wsfe/voucher-pdf failed");
            const message = err instanceof Error ? err.message : String(err);
            return await reply.status(500).send({ ok: false, error: message });
        }
    });
    ...
    app.post("/wsfe/create-voucher", async (request, reply) => {
        ...
        try {
            ...
            request.log.error(err instanceof Error
                ? { errName: err.name, errMessage: err.message }
                : { err: String(err) }, "POST /arca/wsfe/create-voucher failed");
            const message = err instanceof Error ? err.message : String(err);
            return await reply.status(500).send({ ok: false, error: message });
        }
    });
